...
$session = New-PSSession azdc01 -Credential (Get-Credential)
    ...
    if($i -ge 1) { #to give errors in case user does not select .csv file
            ...
            ForEach ($row in $global:users)
                ...
                $currentRowEmail = $row.UserPrincipalName
                     ...
                     Write-Host "Changes may take up to 30mins to propagate..."
                     Write-Host "---------------------------"
...
$lbl_title.text = "Fitzroy Hyper User Control v2.0"
