...
function loadCSV{
    Try{
    Write-Host "$filePath from functon loadCSV" -ForegroundColor Yellow
 $users = Import-CSV $filePath -Header "UserPrincipalName", "Login", "Password"
 $global:users = $users
  }
  Catch{"Failed to import data"}
                  } #-- close function
function Start-log{
...
Get-MsolDomain -ErrorAction SilentlyContinue
    if($?){ }
    Else{
Show-AnyBox -Message "You will be asked to login into Microsfot Online 365" -Buttons "OK" -ContentAlignment "Center"
...
loadCSV
    ...
    ForEach ($row in $global:users)
       ...
       <# $securePassword = ConvertTo-SecureString $currentRowPassword -AsPlainText -Force #not using this line
        Try{        $securePassword = ConvertTo-SecureString $currentRowPassword -AsPlainText -Force}
        Catch [System.Management.Automation.ParameterBindingException] {Write-Host ""}
        Catch{Write-Host ""} #>
...
Show-AnyBox -Message "Accounts on your list are now BLOCKED, please check exceptions.","$global:logPath" -Buttons "OK"
...
Show-AnyBox -Message "Accounts on your list have been UNLOCKED, please check exceptions ","$global:logPath" -Buttons "OK"
...
Show-AnyBox -Message "Accounts on your list were given licensen $license, please check exceptions.","$global:logPath" -Buttons "OK"
...
Show-AnyBox -Message "Accounts on your list had ALL licensed REMOVED, please check exceptions.","$global:logPath" -Buttons "OK"
...
Elseif ($rdn_Button2.Checked  -eq $true ) {Write-Host "2"}
Else {Write-Host "You are not yet connected to Office 365" -foreground Red} 
 }#close function
...
$window.Controls.AddRange(@($lbl_title,$btn_Search,$lbl_filePath,$lbl_Onboarding,$rdn_Button1,$rdn_Button2,$rdn_Button3,$lbl_Offboarding,
