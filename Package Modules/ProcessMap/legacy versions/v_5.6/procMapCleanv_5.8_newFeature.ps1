...
foreach ($x in $data) {
    ...
    if(!($nouser)){if($objectUser -notlike $nul -or $objectUser -notlike ""){
         $trackString = "$($x.objectUser) --> $($x.parentFilePath) --> $($x.processFilePath) --> $($x.objectFilePath)"
         Write-Host "param selected no user" }#close if
            
            }#close if on param switch $nouser
    #if -nouser switch is selected, do not perform the check if objectUser is empty.
    else {Write-Host "[INFO] Importing Rows that dont have an user associated with it" -ForegroundColor Yellow
    $trackString = "$($x.objectUser) --> $($x.parentFilePath) --> $($x.processFilePath) --> $($x.objectFilePath)"}
...
if($nouser){generateTemp1 -nouser}
else{generateTemp1}
