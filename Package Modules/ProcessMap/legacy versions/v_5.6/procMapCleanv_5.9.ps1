...
foreach ($x in $data) {
    ...
    if(!($nouser -or $n)){
        if($objectUser -like $null -or $objectUser -like ""){
         ...
         Else{Write-Host "Row with no user found, not addig to trackString" -ForegroundColor Red}
            
         }#close if on param switch $nouser
             #if -nouser switch is selected, do not perform the check if objectUser is empty.
    else {
        Write-Host "[INFO] Importing Rows that dont have an user associated with it" -ForegroundColor Yellow
        $trackString = "$($x.objectUser) --> $($x.parentFilePath) --> $($x.processFilePath) --> $($x.objectFilePath)"
    }
...
Write-Host "If you run without any switches you will be creating a mindMap that will take the last binary of the path and use to make the connections."
Write-Host "This is to make it easier to read the connections and to help keep the mindMap small, to see the full path on the mindMap use the switch -v" 
Write-Host "- By default the script will check which rows do not have a user associated with the command and if there is none it will skip the record."
Write-Host "If you have a .csv file that you want to connect and do not have a user associated with it use the switch -n`n`n"
...
elseif($nouser -or $n -and $v -or $verbose){generateTemp1 -nouser -verbose}
else{generateTemp1}
