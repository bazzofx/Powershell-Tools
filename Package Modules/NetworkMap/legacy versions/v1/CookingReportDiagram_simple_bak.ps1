...
function generateCsv {
    ...
    ForEach ($x in $data) {
        ...
        $row | Add-Member -MemberType NoteProperty -Name "deviceDirection" -Value $deviceDirection
...
#
# label: SRC:%ip%<br><i style="color:gray;">DNS:%peerHost%</i><br><i style="color:#228B22;fontSize:15;">Destination:%hostName%</i>
##label: ip:%ip%<br>
...
function cleanup {
Write-Host "Starting clean up process."-ForegroundColor Cyan;sleep 1
Remove-Item $allips
Remove-Item $temp1
Remove-Item $merged
...
function main {
Write-Host "SCRIPT STARTED" -ForegroundColor Green;sleep 1
Write-Host "Generating draw.io diagram from data..." -ForegroundColor Green;sleep 1
generateCsv
generateAllIps
mergefiles
generateScript
Write-Host "Script file generate" -ForegroundColor Green

cleanup
Write-Host "SCRIPT COMPLETED" -ForegroundColor Green;sleep 1
}
